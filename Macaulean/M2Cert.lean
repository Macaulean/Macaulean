...
goal's variables (in the request's index order) with integer coefficients, in
Macaulay2's emission order. -/
abbrev CertPoly := Array (Array Nat × Int)

/-- Read a coefficient: `"3"` (an `Int`) or `["3", "1"]` (a `Rat`).  A
denominator other than 1 is an error -- the reflective path's coefficients are
integers, and there is no place to put the scaling. -/
def parseCoeff (j : Json) : Except String Int :=
  match j with
  | .str s => (s.toInt?).elim (.error s!"not an integer: {s}") .ok
  | .arr #[n, d] => do
    let nS ← n.getStr?
    let dS ← d.getStr?
    let some num := nS.toInt? | .error s!"not an integer numerator: {nS}"
    let some den := dS.toInt? | .error s!"not an integer denominator: {dS}"
    if den == 1 then .ok num
    else .error s!"the cofactor has the non-integral coefficient {num}/{den}; \
      `m2cert` builds certificates over the integers"
  | j => Lean.fromJson? j
...
/-- Render a certificate polynomial in `poly_def`'s monomial format:
space-separated `e₁.….eₙ.k` tokens, in the order they arrived. -/
def CertPoly.toSpec (p : CertPoly) (nv : Nat) : String :=
  if p.isEmpty then
    ...
    " ".intercalate (p.toList.map fun (powers, k) =>
      String.intercalate "." ((powers.toList.map toString) ++ [toString k]))
...
unsafe def certify (tacName : Name) (goal : MVarId) (A : Expr) (gens : Array Expr)
    (polyExpr : Expr) : MetaM (Array FVarId × Array String) := do
  ...
  pure (fvars, quotients.map (·.toSpec nv))
...
def suggestionText (specs : Array String) (vars : Array String)
    (native : Bool) (hypNames : Array String) : String :=
...
unsafe def run (ref : Syntax) (tacName : Name) (native suggest : Bool)
  ...
  let (fvars, specs) ← certify tacName goal A gens dividend
  let cofactors ← buildCofactors A fvars specs
  if hypStxs.isNone then
    PolyCert.closeDvd native goal cofactors[0]!
  else
    PolyCert.closeEq native goal hyps cofactors
  replaceMainGoal []
  if suggest then
    let vars ← fvars.mapM fun fv => (varName fv : MetaM String)
    let hypNames := (hypStxs.getD #[]).map fun t => (Syntax.prettyPrint t.raw).pretty
    let text := suggestionText specs vars native hypNames
