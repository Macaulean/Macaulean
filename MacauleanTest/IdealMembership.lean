...
/-! `+cert`: the same goal closed by a kernel-checked linear-combination
certificate — Macaulay2's cofactors enter the proof as `KPoly` data and one
`decide +kernel` evaluation verifies `p = Σ qᵢ·gᵢ`. -/
theorem m2_cert_residual
  (u r k x y z a b c : Rat)
  (ho : (x - u * z) ^ 2 + y ^ 2 - r ^ 2 * z ^ 2 = 0)
  (hi : a ^ 2 + b ^ 2 - c ^ 2 = 0)
  (hpq : x * a + y * b - z * c = 0)
...
#print axioms m2_cert_residual

/--
error: Tactic `m2idealmem` failed: Macaulay2 remainder is nonzero: the target is not in the ideal generated by the hypotheses
...
-/
#guard_msgs in
example (x y : Rat) (h : x ^ 2 = 0) : x + y = 0 := by
  m2idealmem +cert [h]
