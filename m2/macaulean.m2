...
polyDataToRing = (R, data) -> (
    -- coefficients/exponents may arrive as JSON numbers or strings
    toZ := x -> if instance(x, String) then value x else x;
    sum(apply(data, term -> (
        coeff := toZ term#0;
        mon := term#1;
        coeff * product(mon, vp -> R_(toZ vp#0)^(toZ vp#1))
    )))
    )
...
registerMethod(server, "sosCertificate", {"numVars", "polyData"}, (numVars, polyData) -> (
        sosCertificate(numVars, polyData)
    )
)
