...
instance : FromJson Grind.CommRing.Power where
  fromJson? := fun
    | .arr #[x, k] => do
      -- accept both bare JSON numbers and string-encoded numbers
      match x.getNat?, k.getNat? with
      | .ok xnat, .ok knat => pure ⟨xnat, knat⟩
      | _, _ =>
        let xstr ← x.getStr?
        let kstr ← k.getStr?
...
instance : FromJson Term where
  fromJson? := fun
    | .arr #[c, m] => do
      let mon ← fromJson? m
      -- accept both encodings: a bare JSON integer (used e.g. by the SOS
      -- certificate reply) or a string-encoded integer (arbitrary precision)
      match c.getInt? with
      | .ok cint => pure ⟨cint, mon⟩
      | .error _ =>
        let cstr ← c.getStr?
        let some cint := cstr.toInt? |
          throw s!"Expected a String representing an Int {cstr}"
        pure ⟨cint, mon⟩
